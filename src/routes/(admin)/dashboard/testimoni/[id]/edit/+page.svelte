...
<section class="lg:p-10 p-5 w-full ">
    <form action="?/update" method="POST" class="grid lg:gap-5 gap-3 lg:w-1/2 w-full" use:enhance={() => {
      return async({result, update}) => {
        if(result.type === 'success') {
          addToast('Berhasil Update data', 'success')
        ...
        <div>
            ...
            <br>
            <textarea name="komentar" placeholder="Masukan Komentar" class="border mt-2 w-full border-green-400 focus:outline-green-600 p-2 rounded-md" value={data.dataTestimoni?.nama} required></textarea>
        </div>
